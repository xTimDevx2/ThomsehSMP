    ...
    public void checkPlayerAFKStatus(Player player){
        if (lastMovement.containsKey(player)){
            ...
            if (previousData.containsKey(player)){
                ...
                if(wasAFK && !nowAFK){
                    player.setPlayerListName(player.getPlayerListName().replace(" §7§o(afk)", ""));
                    previousData.put(player, false);

                    announceToOthers(player, false);

                }else if(!wasAFK && nowAFK){
                    player.setPlayerListName(player.getPlayerListName() + " §7§o(afk)");
                    previousData.put(player, true);
...
//                    if(!player.equals(target)){
                    if (isAFK){
                        player.sendMessage("§8» §3§o" + target.getDisplayName() + " §fis nu AFK.");
                    }else{
                        player.sendMessage("§8» §3§o" + target.getDisplayName() + " §fis niet meer AFK.");
                        //}
                    }
                });
